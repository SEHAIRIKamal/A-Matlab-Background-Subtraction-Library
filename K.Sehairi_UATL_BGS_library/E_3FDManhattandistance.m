% Three frame diffrence  applied on RGB color images, the frame diffrence are computed using the Manhattan distance
...
for k = 3 : numFrames
    ...
    S1=(abs(J(:,:,1)-I(:,:,1))+abs(J(:,:,2)-I(:,:,2))+abs(J(:,:,3)-I(:,:,3)));
    S2=(abs(J(:,:,1)-L(:,:,1))+abs(J(:,:,2)-L(:,:,2))+abs(J(:,:,3)-L(:,:,3)));

...
%     cd  ( 'C:\dataset2014\dataset\baseline\pedestrians\input')
     cd  ( 'C:\Users\asus\Desktop\work\Video proc\video testes\Motion detection dataset\dataset2014\dataset\baseline\PETS2006\input')
%     % cameraJitter
...
for z=10:numFrames
   ...
   % read the current frame
    ...
    X_t_HSV=padarray(X_t_HSV,[1 1],'replicate','both');
    if z==10
        W=BG_HSV;
        W=padarray(W,[1 1],'replicate','both');
    end
...
%     end
    ...
    % automatic threshold applied on diff and the value plan
...
%     Th2=0.25;
    ...
    fg1 = im2bw(Diff,level);
    
    DiffV=abs(Xt_V-W_V);
    
    level2 = graythresh(DiffV);
    if level2<0.05
        level2=0.10;
    end
    fg2 = im2bw(DiffV,level2);


    for i=2: n-2
        for j=2:m-2
            if ((fg1(i,j)==1) && (fg2(i,j)==1))
                fg(i,j)=1;
            else
                fg(i,j)=0;
                for k=1:3

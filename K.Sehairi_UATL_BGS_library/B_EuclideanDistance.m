% frame diffrence applied on RGB color images, the frame diffrence are
% computed using the Euclidean distance L2 norm
clear all
close all
clc
...
for k = 2 : numFrames
    ...
    % read frames
    firstFrame=imread(['in', num2str(k-1, '%.6d'),'.jpg']);
    secondFrame=imread(['in', num2str(k, '%.6d'),'.jpg']);
    % you can use these commands to read frames
    %firstFrame = imread(fullfile(imageNames{k-1}));
    %secondFrame =imread(fullfile(imageNames{k}));
    ...
    S=sqrt((J(:,:,1)-I(:,:,1)).^2+(J(:,:,2)-I(:,:,2)).^2+(J(:,:,3)-I(:,:,3)).^2);

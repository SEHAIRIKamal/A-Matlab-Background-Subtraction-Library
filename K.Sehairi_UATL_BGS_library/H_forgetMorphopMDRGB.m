...
% forgetting morphological operators applied on RGB color images
clear all
close all
clc
...
for k = 2 : numFrames
    ...
    I=double(Frame);
        ...
        % 
  		alpha = 0.3;
  		if k == 2
  			Background = imread(['in', num2str(k-1, '%.6d'),'.jpg']);
...
%             for i=1:height 
%                 for j=1:width
%                     MaxBG(i,j)=max(I(i,j),MaxBackground(i,j));
%                     minBG(i,j)=min(I(i,j),minBackground(i,j));
%                 end
%             end

            MaxBG(:,:,1)=max(I(:,:,1),MaxBackground(:,:,1));
            MaxBG(:,:,2)=max(I(:,:,2),MaxBackground(:,:,2));
            MaxBG(:,:,3)=max(I(:,:,3),MaxBackground(:,:,3));
            
            minBG(:,:,1)=min(I(:,:,1),minBackground(:,:,1));
            minBG(:,:,2)=min(I(:,:,2),minBackground(:,:,2));
            minBG(:,:,3)=min(I(:,:,3),minBackground(:,:,3));
            
            % update the background
  			MaxBackground(:,:,1) = (alpha * I(:,:,1)) +((1-alpha)* MaxBG(:,:,1));
            MaxBackground(:,:,2) = (alpha * I(:,:,2)) +((1-alpha)* MaxBG(:,:,2));
            MaxBackground(:,:,3) = (alpha * I(:,:,3)) +((1-alpha)* MaxBG(:,:,3));
            
            minBackground(:,:,1) = (alpha * I(:,:,1)) +((1-alpha)* minBG(:,:,1));
            minBackground(:,:,2) = (alpha * I(:,:,2)) +((1-alpha)* minBG(:,:,2));
            minBackground(:,:,3) = (alpha * I(:,:,3)) +((1-alpha)* minBG(:,:,3));
        end
        S=sqrt((MaxBackground(:,:,1)-minBackground(:,:,1)).^2+(MaxBackground(:,:,2)-minBackground(:,:,2)).^2+(MaxBackground(:,:,3)-minBackground(:,:,3)).^2);

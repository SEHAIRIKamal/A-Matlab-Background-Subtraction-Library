% Three frame diffrence applied on RGB color images, the frame diffrence are computed using the Euclidean distance
...
for k = 3 : numFrames
    ...
    S1=sqrt((J(:,:,1)-I(:,:,1)).^2+(J(:,:,2)-I(:,:,2)).^2+(J(:,:,3)-I(:,:,3)).^2);
    S2=sqrt((J(:,:,1)-L(:,:,1)).^2+(J(:,:,2)-L(:,:,2)).^2+(J(:,:,3)-L(:,:,3)).^2);

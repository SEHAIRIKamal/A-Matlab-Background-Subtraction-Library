% Recursive background subtraction method (running average filter) applied
% on color images
...
for k = 2 : numFrames
    ...
    % convert to double
    I=double(Frame);
    % learning (forget) parameter
    alpha = 0.05;
    Th=125;
  	if k == 2
        % read the empty scene frame
        ...
        Iprev=double(Iprev);
        % test all image componenets
        S1=abs(Background(:,:,1)-I(:,:,1));
        S2=abs(Background(:,:,2)-I(:,:,2));
        S3=abs(Background(:,:,3)-I(:,:,3));
        for i=1:height
            for j=1:width
                % update each component separatly
                if (S1(i,j)<Th)
                    Background(i,j,1) = (alpha * Iprev(i,j,1)) +((1-alpha)* Background(i,j,1));
                else 
                    Background(i,j,1) =Background(i,j,1);
                end
                
                if (S2(i,j)<Th)
                    Background(i,j,2) = (alpha * Iprev(i,j,2)) +((1-alpha)* Background(i,j,2));
                else 
                    Background(i,j,2) =Background(i,j,2);
                end
                
                if (S3(i,j)<Th)
                    Background(i,j,3) = (alpha * Iprev(i,j,3)) +((1-alpha)* Background(i,j,3));
                else 
                    Background(i,j,3) =Background(i,j,3);
                end
            end
        end
    end
    S=sqrt((Background(:,:,1)-I(:,:,1)).^2+(Background(:,:,2)-I(:,:,2)).^2+(Background(:,:,3)-I(:,:,3)).^2);

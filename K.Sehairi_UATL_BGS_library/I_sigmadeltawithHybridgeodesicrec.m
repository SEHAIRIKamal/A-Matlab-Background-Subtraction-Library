...
% avec rebouclage et rconstruction  p 136 [Ric06].pdf
clear all
close all
clc
...
% sigma-delta(mesure de vraissemblance du mouvement)
 ...
 BWO=double(BWO);
    ...
    %  geodesic reconstruction defined by the relaxation of the geodesic dilation
    for i=2:height-1 
       for j=2:width-1
            HDilDD0(i,j)=alpha*DD(i,j)+(1-alpha).*max(DD(i,j),HDilDD0(i-1,j));
       end
    end
    for i=2:height-1 
       for j=2:width-1
            HDilDD1(i,j)=alpha*HDilDD0(i,j)+(1-alpha).*max(HDilDD0(i,j),HDilDD1(i+1,j));   
       end
    end
    
    for i=2:height-1 
       for j=2:width-1
            HDilDD2(i,j)=alpha*HDilDD1(i,j)+(1-alpha).*max(HDilDD1(i,j),HDilDD2(i,j-1));   
       end
    end
    
    for i=2:height-1 
       for j=2:width-1
            HDilDD(i,j)=alpha*HDilDD2(i,j)+(1-alpha).*max(HDilDD2(i,j),HDilDD(i,j+1));   
       end
    end
        O=min(BWO,HDilDD);    
...
if k>3
        ...
        se = strel('disk',1);
        erodeE = imerode(E,se);
        dilateL=imdilate(erodeE,se);
        L=min(dilateL,E);
        
        dilateLprev=imdilate(Lprev,se);
        L2=min(L,dilateLprev);
        Lprev=L;
end      
...
if k>3
    imwrite(L2,[pathname,['bin', num2str(k, '%.6d'),'.png']]);
end
    %hold on
    %open(vidObj);

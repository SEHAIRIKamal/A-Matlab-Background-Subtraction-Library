% Three frame diffrence  applied on RGB color images, the frame diffrence are computed using the Chebychev distance
% two possibilities:
% 1) J-I and L-J
% 2) J-I and L-I
...
for k = 3 : numFrames
    ...
    % you can use these commands to read frames
    %firstFrame = imread(fullfile(imageNames{k-2}));
    %secondFrame =imread(fullfile(imageNames{k-1}));
    %thirdFrame =imread(fullfile(imageNames{k}));
    
    % convert to double    
    I=double(firstFrame);
    J=double(secondFrame);
    L=double(thirdFrame);
    ...
    diff2green=J(:,:,2)-L(:,:,2);
    diff2bleu=J(:,:,3)-L(:,:,3);
    % search the max between two matrices then do the same thing with the
    % third one (or change all the matrices to one dimmension)
    S1=max(max(diff1red,diff1green),diff1bleu);
    S2=max(max(diff2red,diff2green),diff2bleu);
